    ...
    class ConsultaRepositoryImpl extends ConsultaRepository {
       ...
       override def createConsulta(consulta: Consulta): Consulta = {
        ...
        val categoria: Categoria=consulta.categoria
         ...
         Logger.info("Guardando consulta"+categoria)
            ...
            //CREATE (pr:Producto {nombre:{pnombre}, descripcion:{descripcion}})//, "pnombre"->producto.nombre, "descripcion"->producto.descripcion
             val allUsuarios= Cypher("MATCH (n:Usuario) WHERE n.user={user} RETURN n.nombre as nombre, n.email as email, n.latitud as latitud, n.longitud as longitud, n.user as user, n.password as password, n.twitter as twitter").on("user"->consulta.usuario.user)().map{     
                case CypherRow(nombre: String, email: String, latitud:BigDecimal,longitud:BigDecimal, user:String, password:String, twitter:String)=>Usuario(nombre, email, latitud, longitud, user, password, twitter,"0412718181")     
            }
            ...
            if(lista.size>0){
                ...
                Logger.info("Se crea el usuario anonimo")
                   val usuario: Usuario = consulta.usuario
                    val result = Cypher("""MATCH (c:Categoria) WHERE c.nombre={catnombre} CREATE (usr:Usuario { nombre: {nombre}, email: {email}, twitter: {twitter}, telefono:{telefono}, latitud:{latitud}, longitud:{longitud}, user:{user},password:{password}}) CREATE (pr:Producto {nombre:{pnombre}, descripcion:{descripcion}}), (pr)-[:PERTENECE]->(c), (usr)-[:BUSCO]->(pr)""").on("catnombre"->categoria.nombre, "nombre"->usuario.nombre, "email"->usuario.email, "twitter"->usuario.twitter, "telefono"->usuario.telefono, "latitud"->usuario.latitud.toDouble, "longitud"->usuario.longitud.toDouble, "user"->usuario.user,"password"->usuario.password, "pnombre"->producto.nombre, "descripcion"->producto.descripcion).execute()
        ...
        override def list(): List[Consulta]={
            ...
            val allConsultas = Cypher("MATCH (cl:Usuario)-[r:BUSCO]->(n:Producto)-[p:PERTENECE]->(c:Categoria) RETURN cl.nombre as nombre, cl.email as cemail, cl.twitter as ctwitter, cl.telefono as ctelefono, cl.latitud as clatitud, cl.longitud as clongitud, id(n) as id, n.nombre as pnombre, n.descripcion as pdescripcion, c.nombre as catnombre, c.descripcion as catdesc")().map { row =>
              (Consulta(Usuario(row[String]("nombre"),row[String]("cemail"),row[BigDecimal]("clatitud"),row[BigDecimal]("clongitud"),row[String]("ctwitter"),row[String]("ctwitter"),row[String]("ctwitter"),row[String]("ctelefono")),(Producto(row[Long]("id"), row[String]("pnombre"),row[String]("pdescripcion"))),Categoria(row[String]("catnombre"),row[String]("catdesc"))))
            }
